 ...
 * @LastEditTime: 2020-07-15 15:30:51
 * @description: 商品详情页
 -->
<template>
  <div class="container">
      ...
      <div class="detail-box" v-show="!isIniting">
        <div class="nav-content row row-center" :class="[headerActive?'':'active']">
          ...
          <nav class="nav-item row row-around" v-show="!headerActive">
            ...
            <label
              class="nav-link"
              :class="[headerTabVal == item.value?'active':'hide']"
              v-for="(item,index) in headerInfo"
              :key="index"
              @click="goAssignBlock(item.value)"
            >
              <input type="radio" :value="item.value" v-model="headerTabVal" />
              {{item.name}}
            </label>
        ...
        <div class="pro-hd">
          <div
            class="pro-hd-title presell-pay-time"
            v-if="promotion&&promotion.type=='PRESELL'&&product.presellVO.restStartAt!=null"
          >
          ...
          <div class="pro-hd-top" v-if="!promotion">
            ...
            <span class="pro-hd-top-price">
              {{product.marketPrice|moneyInt}}.
              <span
                class="pro-hd-top-flog"
              >{{product.marketPrice|moneyFloat}}</span>
            </span>
          ...
          <div class="row row-start">
            <span
              v-if="promotion&&promotion.type=='PRESELL'"
              class="pro-hd-presell-date"
            >{{promotion.shipMonth}}月{{promotion.shipDay}}号发货</span>
          </div>
        ...
        <div class="detail-item activity row row-start" v-if="promotion||ttzPromotion||fullReduces">
          ...
          <span
            class="activity_des text-center"
          >{{promotion?promotionTitle:fullReduces?fullReduces['FULLCUT']?'满减':'满件':'团团赚'}}</span>
          <span
            class="activity_text"
          >{{promotion?(promotion.type!='LIMITBUY'? promotionDes:`每人限购${promotionVo.amount}盒`):fullReduces?fullReduces['FULLCUT']?fullReduces['FULLCUT'][0]['title']:fullReduces['FULLPIECES'][0]['title']:'分享赚取补贴'}}</span>
        </div>
        <!-- 砍价记录 -->
        <div class="bargain-record-box" v-if="promotion&&promotion.type=='BARGAIN'">
          <div class="detail-item bargain-record row row-between">
            ...
            <div
              v-if="!promotionVo.histories||((promotionVo.histories.length||0) != promotionVo.maxTimes)"
            >
              <span
                class="bargain-residue"
              >剩余砍价次数：{{promotionVo.maxTimes - (promotionVo.histories&&promotionVo.histories.length||0)}}</span>
              <a
                href="javascript:void(0)"
                class="bargain-invite-btn vc-font-bold"
                @click="inviteBargainShareUrl"
              >邀请好友砍价</a>
            </div>
            <div v-else>
          ...
          <div class v-if="promotionVo.histories">
            <p
              class="bargain-record_empty text-center row row-center"
              v-if="!promotionVo.histories.length"
            >
              <i class="icon icon-empty_cut vc-line-block"></i>
              <label>还没有砍价记录</label>
            </p>
            <template v-else>
              <div
                class="bargin-record-item row row-between"
                v-for="item in promotionVo.histories"
                :key="item.id"
              >
                <span class="vc-text-hidden">
                  您的好友{{item.userName}}帮您
                  <span class="bargain-discount">砍价{{item.discount}}元</span>
                </span>
          ...
          <div v-else class="evaluate-item row row-center">
            <i class="icon icon-empty_cut vc-line-block"></i>
            <span class="empty-evaluate">还没有砍价记录</span>
          </div>
        </div>
        ...
        <div class="groupon-box" v-if="promotion&&promotion.type=='GROUPON'&&promotionVo.users">
          <div class="detail-item groupon-item row row-between">
            ...
            <div>
              <span
                class="bargain-residue"
              >{{promotionVo.needNumbers==0?'拼团成功':`惊喜${promotionVo.numbers}人团`}}</span>
              <a
                v-if="promotionVo.needNumbers"
                href="javascript:void(0)"
                class="groupon-invite-btn vc-font-bold"
                @click="sharePd"
              >邀请拼团</a>
            </div>
          </div>
          <div class="groupon-invite-map">
            <div
              class="groupon-invite-header"
              :class="[index===0?'active':'other']"
              :style="{backgroundImage:`url(${item.avatar})`,backgroundSize:'cover'}"
              v-for="(item,index) in promotionVo.users"
              :key="item.id"
            ></div>
            <div
              class="groupon-invite-header"
              v-for="item in promotionVo.needNumbers"
              :key="'need'+item"
            ></div>
          </div>
        </div>
        ...
        <div class="recommend-box" v-show="skusList.length" @click="showSkuPopup">
          <div class="recommend-nav row row-between">
            ...
            <div class="row row-end">
              <span
                class="vc-text-hidden sku-current-title vc-line-block"
              >{{skusList.length&&skusList[selectSkuIndex].spec||'请选择规格'}}</span>
        ...
        <div class="recommend-box" ref="EVALUATE">
          <div class="recommend-nav row row-between">
            ...
            <span
              ...
              v-if="product.totalCommentsRecord"
              @click="showAllEvaluate"
            >
              共有{{product.totalCommentsRecord}}条评论
          ...
          <div v-else class="evaluate-item row row-center">
            <i class="icon icon-empty_evaluate vc-line-block"></i>
            <span class="empty-evaluate">还没有评价哦</span>
          </div>
        </div>
        ...
        <div class="detail-item" ref="DETAIL">
          ...
          <lazy-component>
            <div v-html="article" class="detail-content"></div>
          </lazy-component>
        </div>
        <lazy-component>
          <div class="recommend-box" v-if="recommendList.length">
            ...
            <div class="recommend-container">
              <div
                ...
                hover-class="hover"
                @click="goNewPage(d.productId)"
                v-for="(d,i) in recommendList"
                :key="i"
              >
                <div class="recommend-img row row-center">
                  <img alt v-lazy="d.img" class="img" />
                </div>
                <span class="recommend-title">{{d.name}}</span>
                <div class="row row-between">
                  <span class="recommend-price">&yen;{{d.price|moneyTwo}}</span>
                  <span class="recommend-amount">库存{{d.amount}}件</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <van-popup v-model="showDrawer" position="bottom" @closed="isHandleSkuFromItem=false">
          <!--  v-if="mask"  -->
          <div class="i-drawer-close col col-center">
            <i class="icon icon-delete" @click="closeDrawer"></i>
          </div>
          ...
          <div class="i-drawer-container">
            <div class="container-product" v-if="skusList.length">
              <div class="container-product-header">
                ...
                <div class="pro-info">
                  ...
                  <div>
                    <span
                      class="pro-presell"
                      v-if="promotion&&promotion.presellType =='NORMAL'"
                    >&yen;{{skusList[selectSkuIndex]['presellPrice']}}(预售价)</span>
                    <span
                      class="pro-presell"
                      v-else-if="promotion&&promotion.presellType =='DEPOSIT'"
                    >定金&nbsp;&yen;{{skusList[selectSkuIndex]['advancePrice']|moneyTwo}}&nbsp;&nbsp; 尾款&nbsp;&yen;{{skusList[selectSkuIndex]['restPrice']|moneyTwo}}</span>
                  </div>
                  <div>
                    <span
                      class="pro-money"
                      v-if="!promotion||promotion.type!='PRESELL'"
                    >&yen;{{skusList[selectSkuIndex]['showPrice']}}</span>
                    <span
                      class="pro-sku text-line-through"
                      v-if="promotion"
                    >&yen;{{product.marketPrice|moneyTwo}}</span>
                    <span
                      class="pro-sku"
                      v-if="isTwitter"
                    >&nbsp;&nbsp;PV: {{skusList[selectSkuIndex]['showPv']}}</span>
                  </div>
                  <div v-if="limitAmount">
                    <span
                      class="pro-limit-title"
                    >{{'限购' + limitAmount + '件(已购' + (countBuy) + '件)'}}</span>
                  </div>
                  <div v-if="isAnyTtzVip && ttzVipLevel" class="row row-start">
                    ...
                    <template v-if="ttzVipLevel=='ORDINARY'">
                      <span
                        class="pro-money"
                      >{{(skusList[selectSkuIndex]['price'] - skusList[selectSkuIndex]['ordinaryCardRebate'])|moneyTwo}}</span>
                      <span class="ttz-card-name">普卡</span>
                    </template>
                    <template v-else-if="ttzVipLevel=='SILVER'">
                      <span
                        class="pro-money"
                      >{{(skusList[selectSkuIndex]['price'] - skusList[selectSkuIndex]['silverCardRebate'])|moneyTwo}}</span>
                      <span class="ttz-card-name">银卡</span>
                    </template>
                    <template v-else>
                      <span
                        class="pro-money"
                      >{{(skusList[selectSkuIndex]['price'] - skusList[selectSkuIndex]['goldCardRebate'])|moneyTwo}}</span>
                      <span class="ttz-card-name">金卡</span>
                    </template>
                  </div>
              ...
              <div class="container-product-sku">
                <div class="pro-amount-title" v-if="skusList.length>1">规格</div>
              </div>
              <!-- <div > -->
              <div class="container-product-sku" v-if="skusList.length>1">
                <span
                  class="sku-item text-left"
                  :class="[selectSkuIndex==skuIndex?'sku-item-active':'',skuItem.amount==0?'sku-item-empty':'']"
                  v-for="(skuItem,skuIndex) in skusList"
                  :key="skuIndex"
                  @click="selectSku(skuIndex,skuItem.amount)"
                >{{skuItem.spec}}</span>
                <!-- </div> -->
              </div>
              ...
              <div class="container-product-amount" v-if="skusList.length">
                ...
                <div class="pro-amount-content">
                  <van-stepper
                    ...
                    @change="setNumber"
                    :min="1"
                    :max="skusList[selectSkuIndex]['amount']"
                    :disable-plus="buyNumber>=skusList[selectSkuIndex]['amount']"
                  />
    ...
    <van-goods-action safe-area-inset-bottom>
      <van-goods-action-icon
        icon-class="icon icon-car"
        text="购物车"
        @click.native="jumpCart"
        :info="cartCount||''"
        id="buycar"
      />
      <van-goods-action-icon
        :icon-class="isCollection?'icon icon-collect':'icon icon-collect_dis'"
        text="收藏"
        @click.native="updateCollectStatus"
      />
      ...
      <template v-else-if="promotion&&promotion.type=='GROUPON'">
        <van-goods-action-button
          ...
          color="#BF272D"
          text="原价购买"
          @click.native="wantBuy({isBuyForMarketPrice:true})"
        />
        <van-goods-action-button
          v-if="!promotionVo.users"
          type="danger"
          color="#FF6F63"
          :text="activityGrouponId?'参与':'发起拼团'"
          @click.native="wantBuy"
        />
      </template>
      <template v-else>
        <van-goods-action-button
    ...
    <!-- 团团赚协议 -->
    <v-ttzArgeement
      :show="showTtzAgreement"
      @createVipUser="createVipUser"
      @close="showTtzAgreement=false"
    ></v-ttzArgeement>
    <v-cardDialog
      :show="showCardDialog"
      :info="skusList[selectSkuIndex]"
      :url="cardProductUrl"
      @close="showCardDialog=false"
    ></v-cardDialog>
    <!-- 分享操作弹窗 -->
    <van-popup
      v-model="showShare"
      position="bottom"
      :overlay-class="isAndroid?'share-overlay blod':'share-overlay'"
    >
      <div class="share-popup">
        <p class="share-title">分享至</p>
        <p class="share-content">好友通过您分享的链接可购买此商品</p>
        <div v-if="productPackageRebates.length">
          <p class="share-content">累计购买您可以获得补贴哦！</p>
    ...
    <van-popup v-model="showGuideShare" position="top" overlay-class="share-overlay">
      <div @click="showGuideShare=false">
        <img
          :src="shareGuideImg[!promotion?'normal':promotion.type=='BARGAIN'?'bargain':'groupon']"
          alt
          class="share_guide_img"
        />
      </div>
    </van-popup>
    <!-- 销售协议 -->
